...
\begin{document}
    ...
    On fait évoluer le modèle pour voir si on peut changer l'expressivité (avoir tous les langages rationnels, avoir les langages algébriques). \vincent{Remplacer par: On considère différents ensembles d’opérations de bases pour étudier l’expressivité des modèles obtenus.}
    ...
    \end{itemize}
    \vincent{Je n’aime pas trop tes justifications de «pallier à», c’est pas vraiment une histoire de pallier à des manques, mais plutôt des extensions naturelles, je dirai.}
    ...
    \vincent{J’ajouterai dans cette section la définition des ecritures et effacement à droite plutôt que de les introduire en début de section 3. Ça ferait une définition unique de toutes les opérations qui m’a l’air préférable, d’autant que les définitions de configurations, touches, claviers etc, sont communes à tous les modèles, que tu peux, par complétion, intégrer dans la fin de la présente section. Ensuite, dans la section 2, tu peux comme tu le fais actuellement te concentrer sur les modèles idoines.}

    Nous considérons \vincent{l’ensemble d’opérations élémentaires} $S = A \cup \{\retour, \gauche, \droite\}$, où $\retour$ est appelé \emph{retour arrière}, et $\gauche$, $\droite$, \emph{flèche gauche, flèche droite}.
    ...
    \begin{semopel}
        ...
        \vincent{On pourrait dire plutôt : une action est dite efficiente si la configuration produite est différente de la configuration à laquelle l’opération est appliquée.}
    \end{semopel}
    ...
    \vincent{Moui, c’est un peu casse-gueule comme manière de dire, mais je ne vois pas bien mieux là comme-ça. Le point qui me chifonne un peu c’est que tu n’as pas défini «action élémentaire», même si je vois bien ce que tu veux dire. Peut-être peux-tu définir ton induction avec s une opération élémentaire et dire donc que une action est efficiente si et seulement si, dans la définition inductive, toutes les actions sont efficientes (reste le détail de $\varepsilon$, mais y’a moyen d’être clair là-dessus)}
    ...
    \vincent{Je n’aime pas du tout ce paragraphe, particulièrement le terme de saisie qui est informel et non défini. Cela dit, ça serait compliqué de l’ajouter dans les symboles normaux. Je te suggère de d’abord définir le langage des claviers automatiques comme définition de base.
    Ensuite, si tu veux mentionner le symbole $\entree$, je suggèrerai de reformuler le tout comme suit (je jette juste les idées) : on définit des claviers avec des touches de travail et des touches acceptantes (deux ensembles finis), et on considère les mots du claviers comme étant le résultat d’une séquence de touches de travail suivi d’une touche acceptante. Et tu peux mentionner que dans \cite{bible}, les touches acceptantes étaient notées par une action élémentaire $\entree$ que l’on a choisi de ne pas inclure dans notre définition pour avoir une notion simple d’effectivité (parce que c’est le problème principal que je vois à cette action là -- et en fait ça m’a l’air plus propre de tagger les actions comme acceptantes, mais j’admet que je viens juste d’y penser) -- enfin, tu peux toujours trouver une autre manière de dire qu’on mettra quand même $\entree$ en considérant que les actions de cette opération basique sont effectifs.}
    ...
    La \emph{taille} d'un clavier $K$ est définie comme $\sum_{t \in K} (|t| + 1)$ (\vincent{pourquoi +1 ? Accessoirement, je ne suis pas sûr que ce soit la peine de la définir formellement, mais si tu en as besoin, pourquoi pas}).
    On note $\Kinf$ la quantité $\max\limits_{t \in K} |t|$.
    
    Les symboles spéciaux modifient l'expressivité des claviers. \vincent{Tu n’as pas, il me semble, définit cette notion de symbole spéciaux. Du coup, je suggèrerai de définir les classes comme tu le fais en notant qu’elle contiennent toutes les écritures et effacement à gauche, ou alternativement à définir MK comme cet ensemble et noter les autres avec $MK \cup \{...\}$} On va donc définir différentes classes, qui auront différentes propriétés. On les nomme selon les caractères spéciaux autorisés :
    \[
    \begin{array}{ccc}
    ...
    \vincent{Donc j’aime pas «pallier», je dirai donc plutôt : «Une question naturelle consiste à se demander si l’ajout d’états de contrôle modifie l’expressivité du modèle obtenu»}
    ...
    \end{configétats}
    \vincent{À la réflexion, je n’aime pas trop ta notation de configuration étatique qui ne frappe pas immédiatement comme une paire d’un état et d’une configuration, je noterai donc plutôt ça comme $(q,\conf{u}{v})$, pour qu’il soit bien clair que c’est une extension.}
    ...
    On négligera la présence du symbole entrée, car on accepte par état final (on peut en fait simuler la touche entrée en ajoutants des états, voir annexe). \vincent{Et d’ailleurs, cette remarque peut faire écho à celle que je suggère de rajouter à la section précédente : c’est un résultat bien connu que pour les mots finis : accepter par transition finale et par état final, c’est équivalent dans de nombreux modèles, et donc pour les claviers aussi !}
    ...
    \vincent{C’est un peu rapide, mais c’est suffisant. Par contre, il faudrait une séparation visible avec le paragraphe d’après.}
    ...
    \vincent{Il faut ajouter ici qu’on va construire un automate simulant tous les calculs du claviers avec au plus $B(K)$ lettres inutiles et que par le lemme précédent, cela suffira a avoir le même langage -- le point important par rapport à ce que tu dis, c’est que l’automate simule les calculs du claviers.}
    
    On rappelle que toute touche $t$ d'un clavier sans flèche est équivalente à $\retour^r w$ pour un certain $r$ et un certain $w$, qui peuvent être calculés en temps polynomial en la taille de $t$ \vincent{Met ici la référence d’où vient ce résultat (ça vient de \cite{bible}, mais faut le rappeler et mettre le théorème d’où ça vient).}. Sans perte de généralité, on suppose que toutes les touches de notre clavier sont de cette forme, et on note alors $\sharp t = |w| - r$.
    \vincent{J’ai enlevé plusieurs «donc» qui me semblent suspicieux «on suppose donc», je vois pas pourquoi il y a un donc.}

    Formellement, on procède comme suit\footnote{on s'autorise à avoir des transitions dans l'automate étiquetées par des mots. Il n'est pas difficile d'en extraire un automate étiqueté par des lettres.} :
        \begin{itemize}
        ...
        \item on considère $\A$ dont les états sont les états de $K$ dupliqués $2(\mathcal{B}(K) + 1)$ fois : pour $s_\alpha \in Q(K)$, on les note $s_{\alpha, i}$ et $\overline{s_{\alpha, i}}$, $0 \leqslant i \leqslant \mathcal{B}(K)$ \vincent{Pourquoi $s_\alpha$ ? Pourquoi ne pas simplifier la notation en $s$ et avoir donc $(s,i)$ comme nouvel état. Plus les notations sont légères, mieux c’est. De même, que sont ces $\overline{s_{\alpha,i}}$ ? Donne une intuition ici plutôt que après la figure 1.}
        \item pour toute transition $s_{\alpha} \xrightarrow{\retour^r w} s_{\beta}$ de notre clavier à états, on crée dans $\A$ les transitions suivantes (voir figure \ref{ok}):
            \begin{enumerate}
                ...
                \vincent{Deux point un peu gênants ici : les mots, usuellement, ça commence à $0$, pas à $1$. Et ton $j \leq |w|-1$ est suspicieux : tu forces à lire au moins une lettre ici $w[1,1]$, c’est une lettre si je ne me trompe. Par ailleurs, je ne crois pas que tu aies défini cette notation pour les mots (ce qui peut infirmer ma remarque précédente, d’ailleurs)}
    ...
    \end{example}
    Justifions informellement pourquoi notre ensemble de transitions est correct :
    \begin{enumerate}
    ...
    \begin{bisimautomclav}\label{lklak}
        Soit $K$ un clavier de $\QRK$, on a $\L(K) = \L(\A(K))$.
    \end{bisimautomclav}
    \vincent{Le texte de la page précédente et d’ici est un peu décousu (on ne voit pas bien ce que «correct» veut dire).
    Je te suggère de reformuler comme suit : tu cites le lemme 8 d’abord, puis tu enchaînes en disant que la preuve est donnée en appendice, et que tu donnes ici un schéma de preuve qui consiste en  : tout calcul de ton automate correspond à un calcul du clavier à état ayant au plus B(K) lettres inutile à tout moment et vice-versa. Cela se prouve par induction et par ta justification que tes transitions sont correctes.}
    On peut donc affirmer :
    \begin{QRKegalRat}
        $\QRK = \Rat$
    \end{QRKegalRat}
    Le fait que $\mathcal{B}(K)$ soit polynomial en $|K|$ nous permet d'obtenir le corollaire :
    ...
    \vincent{Idem, je n’aime pas le terme de pallier, faire des reproche, etc. On peut simplement parler d’une autre extension naturelle, qui a des similitudes avec les grammaires algébriques (ce qui permettrait d’ailleurs d’introduire subtilement notre intuition).}
    \begin{defopsym}[Opérations symétriques]
        ...
        \end{align*}
    \end{defopsym}
    \vincent{Comme dit plus tôt, je déplacerai ces opération en section 1.}
    ...
    \vincent{Ce paragraphe est étrange pour deux raisons : - le simple fait qu’on soit capable de générer un langage qui n’est pas rationnel suffit à montrer que le langage n’est pas inclus dans les rationnels, donc inutile de parler du problème indécidable (d’autant que ce genre de remarques est casse-gueule), - la vraie raison qui fait qu’on conjecture que c’est inclus dans les algébriques, c’est que syntaxiquement, le clavier est proche d’une grammaire algébrique (linéaire) et qu’on n’a pas trouvé de langage non-algébrique engendrable par un clavier BRK. }
    
    Cependant, même si on peut définir une notion de lettres inutiles (des deux côtés du curseur) similairement à ce qui est fait en section 2, la technique constituant à borner uniformément leur quantité \vincent{pour construire une grammaire algébrique équivalente} (lemme \ref{bk}) ne peut fonctionner \vincent{ne fonctionne pas}.
    ...
    \textit{
        ...
        \vincent{Je suggère de donner des noms aux séquences de transitions inutiles ET aux transitions utiles, ce qui permettra de clarifier le texte après -- et accessoirement de virer l’ambiguïté sur où sont ces transitions utiles dans les pas.}
    ...
    \begin{proof}[du lemme \ref{lklak}]
       ...
       \vincent{Cette histoire de l’automate devine n’est pas méga agréable. Il me semble plus clair et plus carré de repartir du calcul sur le clavier, de prendre son abstraction (celle du lemme 7) et de définir les transitions correspondantes naturellement. Comme c’est construit pile pour ça, tu devrais pouvoir dire que c’est presque syntaxique comme transformation et éliminer cette notion de divination qui n’est jamais agréable.}
    ...
    \end{proof}
    \vincent{Il y a un léger détail : en fait, techniquement, il y a $\varepsilon$ dans l’intersection de tes 2 langages tout le temps. Mais bon c’est un détail qu’on peut évacuer (avec $\entree$ si on veut), et au pire le problème est pas si différent.}
    \clearpage
    \printbibliography
\end{document}