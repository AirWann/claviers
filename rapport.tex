...
\begin{document}
    ...
    Formellement, on procède comme cela : \corto{Précise qu'on s'autorise ici à mettre des mots de plusieurs lettres sur les transitions de l'automate.}
    \begin{itemize}
    ...
    \textit{
        ...
        Le théorème 2.2 de \cite{shortpathOCS} nous donne une borne de $14n^2 + n \max (x_\alpha,x_\beta)$ transitions empruntées\corto{sur la taille maximale nécessaire du compteur pour atteindre un certain état}. Or $x_\alpha,x_\beta < \Kinf$, et les transitions ne peuvent pas être toutes positives, car on doit revenir en dessous de $\Kinf$.
        
        Ainsi, $\mathcal{B}(K) = 9|K|^2$ convient \corto{Précise d'où vient le 9}.
